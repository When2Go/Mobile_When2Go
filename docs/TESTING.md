...
이 문서는 **테스트를 어떻게 쓸지**의 가이드입니다. **언제 쓸지**가 아닙니다.

- TDD로 코드를 작성하면 **그때 만든 테스트가 영구 자산**입니다. PR 직전에 같은 동작에 대한 테스트를 새로 만들지 않습니다 (중복 금지).
- `/review` 단계 5에서 하는 일은 "**기존 테스트 전부 통과 + 신규 로직에 테스트 누락 없음**"의 확인이지, "테스트를 또 작성"하는 게 아닙니다.
- 한 동작 = 테스트 한 묶음. 같은 함수에 두 군데서 테스트 작성 금지.
