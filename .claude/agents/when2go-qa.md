...
두 가지 트리거 모두 **새 컨텍스트**의 서브에이전트로 실행 — 코드 작성 에이전트(ui/logic)와 컨텍스트를 공유하지 않음:

1. **자동** — `github-issue-work` 스킬의 7번 단계가 PR 생성 직전에 Full 모드로 spawn
2. **수동** — 사용자가 `/review` 커맨드로 명시 호출 (Full 또는 Incremental 모드)

자동/수동 모두 동일한 검증 체크리스트·결과 분류 규칙을 따른다. 자동 리뷰가 통과한 뒤에도 사용자가 PR 직전 수동 `/review`를 추가로 돌리는 것은 자유.

**최대 재시도 2회**: 동일 작업을 2번 반려했는데 3번째도 통과 못 하면 사람에게 보고 후 중단. 자동 모드에서는 PR 생성을 멈추고 리포트를 사용자에게 제출.
