...
이 스킬이 생성하는 **모든 GitHub 텍스트**(이슈 코멘트, PR 본문, 이슈 본문)의 평서문은
**"~습니다" 체(합쇼체)** 로 작성합니다. "~다/~한다" 체 금지.

- 적용: 문장 형태의 본문 전부 (요약·설명·체크 항목 설명·리뷰어 참고 등).
- 예외: `[Feature] …` 같은 제목, 표·코드블록·명령어, 명사구 섹션 헤더는 그대로 둡니다.
- 예: "정적 mock 을 휠로 바꿨다" ❌ → "정적 mock 을 휠로 바꿨습니다" ✅
...
7. **자동 코드리뷰** — `Agent` 툴로 `when2go-qa` 서브에이전트를 **새 컨텍스트**에서 spawn해 Full 모드 리뷰 실행:
   ...
   - 결과 처리:
     - **[CRITICAL] 0건** → 8번(PR 생성)으로 진행.
       - **[INFO] 항목이 있으면**: PR 본문에 싣지 않는다. 대신 `AskUserQuestion` 으로 **작업을 수행한 사용자에게만** "이 INFO 작업을 추가로 진행할지" **그 한 가지만** 묻는다(다른 질문 금지). 진행 선택 시 ui/logic 서브에이전트로 처리 후 7번 재실행, 미진행 선택 시 그대로 8번 진행.
       - **[WARNING] 항목**: PR 본문 하단 `## 자동 검사 결과` 표에만 반영(별도 상세 섹션·판정 줄 금지).
       - PR 본문에 `[CRITICAL]/[WARNING]/[INFO] 건수 — 머지 가능` 판정 줄과 `### INFO 상세` 섹션을 **절대 넣지 않는다** (`docs/PR-writing-guide.md` 자동 리뷰 결과 첨부 규칙).
...
8. PR 생성 — 이슈 close는 하지 않음 (PR Squash Merge 시 자동 close). PR 본문 하단에 자동 리뷰 결과 첨부
   ...
   - **PR 본문은 `docs/PR-writing-guide.md` 규칙을 반드시 따른다**:
     - 평서문은 "~습니다" 체 (위 [문체 규칙](#문체-규칙-필수))
     - 맨 위에 한 줄 요약
     - 변경 사항은 라이브러리 이름 나열 대신 "무엇을 / 왜 / 어떻게 쓰는지" 순으로 평이한 한국어
     - "직접 확인하는 법"은 앱에서 확인할 체크 항목만. `npm install`/`npx expo start`/`npm test` 같은 범용 명령 블록은 항상 제외 (이 변경에만 필요한 특수 명령이 있을 때만 기재)
     - 자동 리뷰 결과는 표로 정리
